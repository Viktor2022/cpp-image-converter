...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    row_stride = image.GetWidth() * cinfo.input_components; /* JSAMPLEs per row in image_buffer */

    while (cinfo.next_scanline < cinfo.image_height) {
        std::vector<JSAMPLE> tempBuf;
        tempBuf.reserve((image.GetWidth() * 3));
        
        const Color *line = image.GetLine(cinfo.next_scanline);
        for (int w = 0; w < image.GetWidth(); w++)
        {
            tempBuf.push_back(static_cast<JSAMPLE>(line[w].r));
            tempBuf.push_back(static_cast<JSAMPLE>(line[w].g));
            tempBuf.push_back(static_cast<JSAMPLE>(line[w].b));
        }

        buffer[0] = tempBuf.data();
